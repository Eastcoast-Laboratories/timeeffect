<!-- user/delete.ihtml - START -->

<?php

	if(isset($user) && is_object($user)) {

		$uid				= $user->giveValue('id');

		$username			= $user->giveValue('username');

		include($GLOBALS['_PJ_root'] . '/templates/user/options/delete.ihtml.php');

	} else {

		return;

	}

?>

	<FORM ACTION="<?php print $GLOBALS['_PJ_user_script']; ?>" METHOD="<?php if(!empty($GLOBALS['_PJ_form_method'])) echo $GLOBALS['_PJ_form_method']; ?>">

	<INPUT TYPE="hidden" NAME="uid" VALUE="<?php if(isset($uid)) echo $uid; ?>">

	<INPUT TYPE="hidden" NAME="delete" VALUE="1">

	<CENTER>

	<TABLE	WIDTH="90%"

			BORDER="<?php print($_PJ_inner_frame_border); ?>"

			CELLPADDING="<?php print($_PJ_inner_frame_cellpadding); ?>"

			CELLSPACING="<?php print($_PJ_inner_frame_cellspacing ); ?>">

		<TR>

			<TD CLASS="content">

			<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0">

				<TR>

					<TD CLASS="MessageAsk" COLSPAN="2"><?php if(!empty($GLOBALS['_PJ_strings']['ask_user_delete'])) echo $GLOBALS['_PJ_strings']['ask_user_delete'] ?></TD>

				</TR><TR>

					<TD ALIGN="left"><INPUT CLASS="FormSubmit" TYPE="SUBMIT" NAME="cancel" VALUE="<< <?php if(!empty($GLOBALS['_PJ_strings']['cancel'])) echo $GLOBALS['_PJ_strings']['cancel'] ?>"></TD>

					<TD ALIGN="right"><INPUT CLASS="FormSubmit" TYPE="SUBMIT" NAME="confirm" VALUE="<?php if(!empty($GLOBALS['_PJ_strings']['delete'])) echo $GLOBALS['_PJ_strings']['delete'] ?> >>"></TD>

				</TR>

			</TABLE></TD>

		</TR>

	</TABLE>

	</CENTER>

	</FORM>

<!-- user/delete.ihtml - END -->

